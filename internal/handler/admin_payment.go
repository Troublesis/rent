...
func paymentToRow(payment model.Payment, now time.Time) paymentRow {
	...
	// Deposit-refund rows store a negative amount so a refunded deposit deducts
	// from income via the shared SUM; display the absolute value instead.
	displayFen := payment.Amount
	if payment.Type == model.PaymentTypeDepositRefund && displayFen < 0 {
	...
	return paymentRow{
		...
		AmountText:        service.FormatFen(displayFen),
		AmountYuan:        service.FormatFenAsInputValue(displayFen),
		PayDate:           payment.PayDate,
		PayDateLabel:      formatDisplayDate(payment.PayDate),
		NextDueLabel:      nextLabel,
		Paid:              payment.Paid,
		PaidAtLabel:       paidAtLabel,
		StatusLabel:       paymentStatusLabelForType(payment.Type, payment.Paid),
...
func paymentToAPIItem(payment model.Payment, now time.Time) paymentAPIItem {
	...
	// Deposit-refund rows store a negative amount; display the absolute value.
	displayFen := payment.Amount
	if payment.Type == model.PaymentTypeDepositRefund && displayFen < 0 {
		displayFen = -displayFen
	}
...
// paymentStatusLabelForType returns a status label appropriate to the payment
// type: deposit refunds read 已退还/未退 instead of 已付款/未付款.
func paymentStatusLabelForType(paymentType string, paid bool) string {
	if paymentType == model.PaymentTypeDepositRefund {
		if paid {
			return "已退还"
		}
		return "未退还"
	}
	return paymentStatusLabelText(paid)
}
