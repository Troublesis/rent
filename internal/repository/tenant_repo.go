...
func (r *TenantRepository) ListTenants(filter TenantFilter) ([]model.Tenant, error) {
	...
	if strings.TrimSpace(filter.Query) != "" {
		...
		joinedRooms = true
		query = query.Where("tenants.name LIKE ? OR tenants.phone LIKE ? OR rooms.room_no LIKE ? OR rooms.title LIKE ? OR (tenants.name || ' · ' || rooms.room_no || ' · ' || rooms.title) LIKE ? OR (tenants.name || ' - ' || rooms.room_no || ' - ' || tenants.phone) LIKE ?", like, like, like, like, like, like)
	}
